...
class SupertaggerTrainer(object):
    ...
    def train(self, config, data):
        initializer = tf.random_uniform_initializer(-config.init_scale,
                                                    config.init_scale, seed=config.seed)
        ...
        with tf.Session() as session:
            ...
            with SupertaggerEvaluationContext(session, data.dev_data, dev_model, self.writer) as eval_context:
                ...
                while not eval_context.stop:
                    with Timer("Epoch {}".format(epoch)) as timer:
                        ...
                        for i,(x,y,num_tokens,mask) in enumerate(train_batches):
                            ...
                            _, cost, reg = session.run([train_model.optimize, train_model.cost, train_model.regularization], {
                                ...
                                train_model.input_dropout_probability: train_model.config.input_dropout_probability,
                                train_model.dropout_probability: train_model.config.dropout_probability
                            })
                            train_cost += cost
                            train_reg += reg
                            if i % 10 == 0:
